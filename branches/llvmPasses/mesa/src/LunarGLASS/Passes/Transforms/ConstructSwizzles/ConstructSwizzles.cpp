...
namespace {
    ...
    void InitSOp(SwizzleOp &sop, InstVec &vec)
    {
    ...
    void BuildSwizzleOp(InstVec &vec, SwizzleOp &sop)
    {
        // Initialize
        InitSOp(sop, vec);
    ...
    void InsertSwizzleIntrinsic(InstVec &vec, Instruction *newInst, BasicBlock &bb)
    {
    ...
    // Group instructions into the individual swizzles, placing them in result
...
bool ConstructSwizzles::runOnFunction(Function &F)
{
    ...
    for (Function::iterator bb = F.begin(), ebb = F.end(); bb != ebb; ++bb) {
        ...
        for (GroupVec::iterator gI = groupVec.begin(), gE = groupVec.end(); gI != gE; ++gI) {
            ...
            VERBOSE(PrintSwizzleIntrinsic(*inst));
